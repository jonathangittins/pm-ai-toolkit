...
> **Forked from** Ryan Singer's [shaping-skills](https://github.com/rjs/shaping-skills) with an explore-exploit lens added to the Pre-work section. Check upstream periodically for updates.
...
#### Explore-Exploit Lens

When comparing options in the Pre-work table, score each on two dimensions -- not just signal strength:

| Dimension | What it measures | High score means |
|-----------|-----------------|------------------|
| **Expected value** | What you know -- customer demand, revenue signal, competitive pressure | Clear, evidence-backed payoff |
| **Uncertainty** | What you don't know -- could a small investment dramatically change your estimate? | A cheap pull (spike, POC, design partner) could reveal something big |

Options with high expected value are obvious candidates. But options with **high uncertainty and plausible upside** also deserve consideration -- a small investment to resolve that uncertainty may be the best use of resources.

Watch for options in the **moderate middle** -- moderate confidence, moderate upside, low uncertainty. These feel productive but won't teach you anything or move the needle.

#### Grounding with Competitive Intelligence

When scoring expected value, check competitive intelligence data for evidence that strengthens or weakens the case for each option:

- Search for loss patterns related to the option (closed-lost deals, competitor mentions)
- Look at structured deal data -- how many deals lost, why, to whom

This is not required for every frame -- use it when the conversations reference competitive pressure or lost deals, or when expected value scores feel like guesses rather than evidence. CI data belongs in the Pre-work table's "Signal strength" column, not in the Source section (which is reserved for what was actually said in conversations).

If the selected option is an **exploration** (high uncertainty), the frame should note:
- What's the cheapest way to validate the core assumption before committing to a full shape?
- What signal would tell you to proceed vs. stop?

This isn't a gate -- it's a flag. Explorations are good. But they should be *designed* as explorations, not accidentally treated as safe bets.
...
| Option | What it does | Who benefits | Signal strength | Uncertainty |
|--------|-------------|--------------|-----------------|-------------|
| **A. [Name]** | ... | ... | ... | ... |
| **B. [Name]** | ... | ... | ... | ... |
...
[If A is an exploration: **Validation note:** A is high-uncertainty. Before committing to a full shape, [cheapest test to resolve the core assumption]. Proceed if [signal]; stop if [signal].]
